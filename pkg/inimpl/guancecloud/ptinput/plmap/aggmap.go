...
import (
	...
	"github.com/GuanceCloud/platypus/pkg/hash"
	"github.com/GuanceCloud/platypus/pkg/inimpl/guancecloud/ptinput/utils"
...
func NewAggBuks(upFn UploadFunc, globalTags map[string]string) *AggBuckets {
	return &AggBuckets{
		uploadDataFn: upFn,
		data:         map[point.Category]map[string]*bucket{},
		gTags:        globalTags,
...
func newBucket(cat point.Category, name string, interval time.Duration,
	count int, keepValue bool, uploadFn UploadFunc, gTags map[string]string,
) *bucket {
	return &bucket{
		bukName:    name,
		interval:   interval,
...
func conv2Pt(b *bucket, tagsName []string, aggTF *aggFields) (*point.Point, bool) {
	...
	if len(b.globalTags) > 0 {
		opt = append(opt, point.WithExtraTags(b.globalTags))
	}
	opt = append(opt, utils.PtCatOption(b.category)...)
