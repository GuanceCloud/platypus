...
func (db *mockDB) Get_all(ip string) (ip2location.IP2Locationrecord, error) { //nolint:stylecheck
...
func TestIPloc(t *testing.T) {
	openDB = mockOpenDB
	iplocInstance := &IPloc{}
	datadir, err := os.MkdirTemp(os.TempDir(), "iploc")
	if err != nil {
	...
	err = os.WriteFile(filepath.Join(iplocDir, iplocFile), []byte(""), fs.ModePerm)
	if err != nil {
		t.Fatalf("iploc file create errror")
	}

	err = os.WriteFile(filepath.Join(iplocDir, ispFile), []byte("221.0.0.0/13 unicom"), fs.ModePerm)
	if err != nil {
